...
function setup_env_vars() {
  ...
  declare -g DATABASE_ALREADY_CONFIGURED

  ORACLE_VERSION=$(sqlplus -version | grep "Release" | awk '{ print $3 }')
  declare -g ORACLE_VERSION

  if [ -f "${ORACLE_BASE}/dbconfigured" ]; then
    DATABASE_ALREADY_CONFIGURED="true";
  fi
  ...
  if [ -z "${DATABASE_ALREADY_CONFIGURED:-}" ]; then
    ...
    echo "CONTAINER: database already configured."
  fi;
}
...
if healthcheck.sh; then
  ...
  if [ -z "${DATABASE_ALREADY_CONFIGURED:-}" ]; then
    ...
    touch "${ORACLE_BASE}/dbconfigured"
